/* === WeedTracker V61 Pilot — storage.js ===
   Local DB + rolling backups + seed data (weeds, chems)
*/
...
function ensureDB(){
  ...
  if (!db.weeds.length){
    db.weeds = [
      // header is also enforced by apps.js (ensureNoxiousHeader) but we include it here too
      "⚠ Noxious Weeds",
...
// auto-offer restore if DB looks empty but backups exist
